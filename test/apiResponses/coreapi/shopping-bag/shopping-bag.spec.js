...
describe('It should return the Shopping Bag Json Schema', () => {
      ...
      shoppingBag.products.forEach((product) => {
        if (product.productId === products.productsSimpleId.productId) {
          ...
          const shoppingBagProductsWasPriceSchema = {
            ...
            optional: ['ageVerificationRequired'],
            properties: {
              ageVerificationRequired: booleanOrString,
              assets: arrayType(4),
              attributes: objectType,
  ...
  describe('Shopping Bag DELETE Product', () => {
    ...
    it(
      ...
      async () => {
        const deleteItemWithInvalidOrderItemId = await removeItemFromShoppingBag(
          newAccount.jsessionid,
          shoppingBag.orderId,
          ''
        )
  ...
  */
  ...
  describe('Shopping Bag POST Add Promotion Code', () => {
    it(
      ...
      async () => {
        ...
        expect(addPromotionCode).toMatchSchema(shoppingBagSchema)
      },
      30000
    )

    it('Shopping Bag Promotion Code => Promotions schema', () => {
    ...
    it(
      ...
      async () => {
        ...
        try {
          validPromotionCode = await promotionCode(newAccount.jsessionid)
        } catch (e) {
    ...
    it(
      ...
      async () => {
        ...
        try {
          addInvalidPromotionCode = await promotionCode(
            newAccount.jsessionid,
            'orderSummary',
            'NO_CODE'
          )
  ...
  describe('Shopping Bag POST Promotion Code Already Redeemed', () => {
    ...
    beforeAll(async () => {
      ...
      await payOrder(newAccount.jsessionid, shoppingBag.orderId, 'VISA')
      await addItemToShoppingBag2(
        newAccount.jsessionid,
        products.productsSimpleId
      )
  ...
  describe('Shopping Bag Transfer POST Request', () => {
    beforeAll(async () => {
      newAccount = await createAccount()
      transferBagResponse = await transferShoppingBag(newAccount.jsessionid)
    }, 30000)

    it('Shopping Bag Transfer Response', () => {
      ...
      expect(transferBagResponse).toMatchSchema(shoppingBagTransferSchema)
    })
  })
})
