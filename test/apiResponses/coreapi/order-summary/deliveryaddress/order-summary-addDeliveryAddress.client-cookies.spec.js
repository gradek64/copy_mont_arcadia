...
describe('Orders Summary - delivery dddress', () => {
  ...
  beforeAll(async () => {
    ...
    const payOrderResponse = await payOrder(cookies, orderId, 'VISA')
    cookies = mergeCookies(payOrderResponse)
    const shoppingBagTwo = await addItemToShoppingBagResponse(
      cookies,
      products.productsSimpleId
    )
    cookies = mergeCookies(shoppingBagTwo)
    ...
    cookies = mergeCookies(addNewDeliveryAddress)
